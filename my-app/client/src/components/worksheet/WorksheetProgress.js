...
class WorksheetProgress extends React.Component {
    ...
    render () {
        ...
        const completedNames = courseNames && courseNames.filter((name)=> courses[name].grade);
        const {requirements} = this.props;
        const progress = Math.round(completedNames && completedNames.length/requirements.length * 100);
