    ...
    onSubmit = () => {
        ...
        if (Object.keys(error).length === 0 && error.constructor === Object) {
            doSignup(this.state.data)
                .then((data) => {
                    this.setState({ registrationSuccess : "Success! Redirecting to the login page." });
                    setTimeout(() => {
                        this.props.submit();
                    }, 1500);
                })
                .catch(function (e) {
                    console.log("failed");
                    this.setState({ registrationError : e && e.response && e.response.data && e.response.data.message});
                }.bind(this));
        }
    };
